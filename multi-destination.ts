...
import { configure as legacyConfigure } from "etcher-sdk/build/source-destination/configured-source/configure"
import { ConfigureFunction } from "etcher-sdk/build/source-destination/configured-source/configured-source"
...
async function openUrl(
  ...
  write: boolean,
  direct = true
): Promise<sourceDestination.Http | sourceDestination.File | sourceDestination.BlockDevice> {
  ...
  if (plat === "win32" && lowercaseUrl.startsWith("\\\\.\\physicaldrive")) {
    ...
    if (device !== undefined && device instanceof sourceDestination.BlockDevice) {
      device.oWrite = write
  ...
  if (stats !== undefined) {
    if (stats.isBlockDevice() || (stats.isCharacterDevice() && plat === "darwin")) {
      const device = Array.from(deviceScanner.drives.values()).find((d) => {
...
async function main({
  ...
  const destinationDrives = await Promise.all(devices.map((d) => openUrl(d, deviceScanner, true)))
...
const argv = require("yargs").command("$0 <sourceImage> [devices..]", "Write the sourceImage on all devices.", (yargs: Argv) => {
