  ...
  if (hasTotal) {
    ...
    function update(progress: multiWrite.MultiDestinationProgress) {
      spinner.setSpinnerTitle(
        `${title}, ${progress.bytes} bytes, current: ${bytesToMebibytes(progress.speed)} MiB/s, average: ${bytesToMebibytes(progress.averageSpeed)}`
      )
    }
